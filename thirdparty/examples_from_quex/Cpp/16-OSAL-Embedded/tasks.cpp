 ...
 *       couple of retries, or the token TERMINATION is found.                */
{
    ...
    int32             status;
    int               retry_n;
    quex::ByteLoader*  byte_loader = quex::ByteLoader_OSAL_new_from_file_name(file_name);
    ...
    tiny  tlex(byte_loader, /* Converter */NULL);
 ...
 * EXIT: Receiption of token 'TERMINATION'.                                   */
{
    ...
    token.text = (tiny_lexatom_t*)quex::MemoryManager_allocate(64, E_MemoryObjectType_TEXT);
