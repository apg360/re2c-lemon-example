...
void ReportVcgGraph(/*struct lemon */); 
...
char **argv;
{
  ...
  static struct s_options options[] = {
    ...
    {OPT_FLAG, "v", (char*)&vcggraph, "Output VCG Graph."}, 
  ...
  if( rpflag ){
    ...
    ** occur at the end. */
	...
	if( vcggraph )		ReportVcgGraph(&lem); 
...
/* The given config will be written to the file pointer, used for the VCG Graph  */
void   ConfigFileOut(cfp,fp)
struct config *cfp;
FILE *fp;
{
  struct rule *rp;
...
/* Generate the VCG graph  */
void ReportVcgGraph(lemp)
struct lemon *lemp;
{
 int vCurStateId=0;
 struct state *stp;
 ...
 FILE *fp;
   ...
   fp = file_open(lemp,".vcg","wb");
   fprintf(fp,"graph: { \n");
   fprintf(fp,"display_edge_labels: yes \n");
   fprintf(fp,"priority_phase:	yes \n");
   fprintf(fp,"port_sharing:	no \n");
   fprintf(fp,"crossing_weight:	median \n");
   fprintf(fp,"splines:	yes \n");
   fprintf(fp,"layoutalgorithm: mindepth \n");

   for( vCurStateId=0;  vCurStateId <lemp->nstate; vCurStateId++) {
    stp = lemp->sorted[vCurStateId];
  
	fprintf(fp,"node: { \n");
	fprintf(fp,"title: \"%d\" \n",vCurStateId);
	fprintf(fp,"label:  \" ");

    for(conf1 =stp->cfp; conf1;conf1= conf1->next){
	ConfigFileOut(conf1,fp); 	
    }  
	fprintf(fp," \"} \n");


    for(act1=stp->ap; act1;act1 = act1->next){ 

		if(act1->type==0){
			fprintf(fp,"edge: { \n");
			fprintf(fp,"sourcename: \"%d\"  \n ",vCurStateId);
			fprintf(fp,"targetname: \"%d\" \n",act1->x.stp->statenum);
			fprintf(fp,"label: \"%s\" \n", act1->sp->name);
			fprintf(fp,"color: blue \n");
			fprintf(fp,"textcolor: black \n");
			fprintf(fp,"}\n ");
		}
